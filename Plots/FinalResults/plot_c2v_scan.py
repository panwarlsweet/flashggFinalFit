...
for ic2v in range(0,rew_dict['Nc2v']):
		c2v = rew_dict['c2vmin'] + ic2v*rew_dict["c2vstep"]	
		c2v_str = ("{:.6f}".format(c2v)).replace('.','d').replace('-','m') 
		fname = options.indir + '/' + 'higgsCombine_c2v_%s_%s.AsymptoticLimits.mH125.root'%(c2v_str,options.outtag)
		vals  = getVals(fname)
		if vals==-1 : continue
		if options.nlo :  
			xs = coupling_xs[ic2v]
...
c1.Print('%s/c2v_scan_%s_to_%s.pdf'%(options.outdir,options.outtag,options.whatToFloat), 'pdf')
