...
int main(int argc, char *argv[]){
	...
	float minSigFitMX = 200;
	float maxSigFitMX = 1200;
	int nbins = 150; // 5 GeV bin 
	if (massY_ == 300)  {minSigFitMX = 450; nbins = 150;}
	else if (massY_ == 400) { minSigFitMX = 550; nbins = 130;}
	else if (massY_ == 500) { minSigFitMX = 650; nbins = 110;}
	else if (massY_ == 600) { minSigFitMX = 750; nbins = 90;}
	else if (massY_ == 700) { minSigFitMX = 850; nbins = 70;}
	else if (massY_ == 800) { minSigFitMX = 950; nbins = 50;}
	...
	for (auto &iproc:procs_) {
      ...
      WSTFileWrapper * w_original_all;
		...
		RooRealVar* MX  = (RooRealVar*)w_original->var("MX");
	...
	//	if (iproc == signalproc_)  {
		...
		for (int ic = 0; ic < NCAT; ++ic)
		{
			...
			if (!(mergeYearsStr_.empty())) {
				for (unsigned int iyear=0; iyear< mergeYears_.size();++iyear){
					...
					if (iyear==0) sigToFitAllYears[ic] = scaleWeight(tmp,MX,weight,(iproc+"_YearsMerged_13TeV_125_"+icat).c_str(),lumiYears_[iyear]);
					else sigToFitAllYears[ic]->append(*(scaleWeight(tmp,MX,weight,(iproc+"_YearsMerged_13TeV_125_"+icat).c_str(),lumiYears_[iyear])));
				}
			}
		...
		for (int ic = 0; ic < NCAT; ++ic)
		{
			...
			if(iproc.find("ggh") != string::npos || iproc.find("qqh") != string::npos  || iproc.find("bbh") != string::npos ) {
					MXSig[ic] = new RooBernstein(("MXHig_"+iproc+"_cat"+std::to_string(c)).c_str(),"",*MX,
					RooArgList( *w->var( ("MX_hig_par1_"+iproc+"_cat"+std::to_string(c)).c_str()) ,
							*w->var(("MX_hig_par2_"+iproc+"_cat"+std::to_string(c)).c_str()) ,//))); //,
							*w->var(("MX_hig_par3_"+iproc+"_cat"+std::to_string(c)).c_str() )));
					w->import(*MXSig[ic]);
			}
			else MXSig[ic] = (RooAbsPdf*) w->pdf(("MX"+proc_type_upper+iproc_type+"_cat"+to_string(c)).c_str());
			...
			if ((c==10) || (c==11)) {
				MX->setRange((iproc+"CutRange").c_str(),90.,maxSigFitMX);
	...
	//		normalization_cat = sigToFit[ic]->sumEntries()/1000.; //as for Hgg use fb
			...
			if (proc_type == "sig")  ((RooRealVar*) w->var(("MX_"+proc_type+"_m0"+iproc_type+"_cat"+to_string(c)).c_str()))->setVal(125.);
			if ((mergeFitMVAcats_) && (NCAT==nCats_)){  //only works in case the set of categories is complete
				MXSig[ic]->fitTo(*sigToFitMVA[categories_scheme[ic]],Range((proc_type_upper+"FitRange").c_str()),SumW2Error(kTRUE),PrintLevel(3));
			} else if (!mergeYearsStr_.empty()){  
				MXSig[ic]->fitTo(*sigToFitAllYears[ic],Range((proc_type_upper+"FitRange").c_str()),SumW2Error(kTRUE),PrintLevel(3));
			} else 
				MXSig[ic]->fitTo(*sigToFit[ic],Range((proc_type_upper+"FitRange").c_str()),SumW2Error(kTRUE),PrintLevel(3));

			//fitResult[ic] =  (RooFitResult*)MXSig[ic]->fitTo(*sigToFit[ic],Range((proc_type_upper+"FitRange").c_str()),SumW2Error(kTRUE),PrintLevel(3))->Clone();

			RooArgSet *sigParams = 0;
			sigParams = (RooArgSet*) MXSig[ic]->getParameters(RooArgSet(*MX));
			...
			for (TObject *a = iter->Next(); a != 0; a = iter->Next())
			{
				...
				//boost::replace_all(newVarName,"MX", "CMS_hbb_mass"); //will do this later
			...
			//  w->import(*w->var("MX"), RenameVariable("MX","CMS_hbb_mass" ) );//will do this later
			//   wAll->import(*w->var("MX"), RenameVariable("MX","CMS_hbb_mass" ));  //will do this later if needed
			...
			double putyear = 1;	
			bool calc_width=1;
			TH1F *h = new TH1F(("h_"+iproc+"_"+year_+"_"+icat).c_str(),("h_"+iproc+"_"+year_+"_"+icat).c_str(),nbins,minSigFitMX,maxSigFitMX);
			...
			RooDraw(can,h,MX->frame(),((RooDataHist*)sigToFit[ic]),MXSig[ic],iproc,(icat),putyear,calc_width, thisSigRange,fwhmRange);
			...
			if ((mergeFitMVAcats_) && (NCAT==nCats_)){  //only works in case the set of categories is complete
				TH1F *hMVA = new TH1F(("hMVA_"+iproc+"_"+year_+"_"+icat).c_str(),("hMVA_"+iproc+"_"+year_+"_"+icat).c_str(),nbins,minSigFitMX,maxSigFitMX);
				MXSig[ic]->fillHistogram(hMVA,RooArgList(*MX),sigToFitMVA[categories_scheme[ic]]->sumEntries());
				TH1F *hMVA_fine = new TH1F(("hMVA_"+iproc+"_"+year_+"_"+icat+"_fine").c_str(),("hMVA_"+iproc+"_"+year_+"_"+icat+"_fine").c_str(),nbins*50,minSigFitMX,maxSigFitMX);
				MXSig[ic]->fillHistogram(hMVA_fine,RooArgList(*MX),sigToFitMVA[categories_scheme[ic]]->sumEntries());
				thisSigRange = getEffSigma(MX,MXSig[ic]);
				fwhmRange = getFWHM(hMVA_fine);		


				TCanvas* canMVA = new TCanvas("canMVA","canMVA",650,650);
				RooDraw(canMVA,hMVA,MX->frame(),((RooDataHist*)sigToFitMVA[categories_scheme[ic]]),MXSig[ic],iproc,("MVA "+to_string(categories_scheme[ic])),putyear,calc_width, thisSigRange,fwhmRange);
			...
			if (!mergeYearsStr_.empty()){  
				TH1F *hAllYears = new TH1F(("hAllYears_"+iproc+"_"+year_+"_"+icat).c_str(),("hAllYears_"+iproc+"_"+year_+"_"+icat).c_str(),nbins,minSigFitMX,maxSigFitMX);
				MXSig[ic]->fillHistogram(hAllYears,RooArgList(*MX),sigToFitAllYears[ic]->sumEntries());
				TH1F *hAllYears_fine = new TH1F(("hAllYears_"+iproc+"_"+year_+"_"+icat+"_fine").c_str(),("hAllYears_"+iproc+"_"+year_+"_"+icat+"_fine").c_str(),nbins*50,minSigFitMX,maxSigFitMX);
				MXSig[ic]->fillHistogram(hAllYears_fine,RooArgList(*MX),sigToFitAllYears[ic]->sumEntries());
				thisSigRange = getEffSigma(MX,MXSig[ic]);
				fwhmRange = getFWHM(hAllYears_fine);		

				TCanvas* canAllYears = new TCanvas("canAllYears","canAllYears",650,650);
				RooDraw(canAllYears,hAllYears,MX->frame(),((RooDataHist*)sigToFitAllYears[ic]),MXSig[ic],iproc,(icat),putyear,calc_width, thisSigRange,fwhmRange);
	...
	std::cout<<"All fits have finished, now merge the workspaces : mergeWorkspace.py workspace_out_MX.root *.root"<<std::endl;	
}
