...
HHReweight5D::HHReweight5D(std::string coeffFile, const TH2* hInput,TString outputDirName, bool useAbsEta)
{
    outputDir_ = outputDirName;
    readInputFile(coeffFile); // initialize the reweight parameters from the txt input
    if (DEBUG) cout << " -- Successfully read the the input file" << endl;

    // clone the input histogram
    TH2* cloneH = (TH2*) hInput->Clone("h_input");
    if (DEBUG) cout << " -- Successfully got the histogram" << endl;
...
void HHReweight5D::readInputFile(std::string coeffFile)
{
    ...
    for (uint ic = 0; ic < NCOEFFSA; ++ic)
    {
      ...
      h_A_vec_.at(ic) = std::make_shared<TH2F> (name.c_str(), name.c_str(), nbins_mHH, binning_mHH, nbins_cth, binning_cth );
    }

    h_SM_    = std::make_shared<TH2F> ("h_SM", "h_SM",       nbins_mHH, binning_mHH, nbins_cth, binning_cth );
    h_sumv1_ = std::make_shared<TH2F> ("h_sumv1", "h_sumv1", nbins_mHH, binning_mHH, nbins_cth, binning_cth );
    ...
    TFile* fOut = TFile::Open(outputDir_ + "HHReweight_histograms.root", "recreate");
    fOut->cd();
    for (uint ic = 0; ic < NCOEFFSA; ++ic)
    {
