    ...
    for num,f in enumerate(input_files):
      ...
      for cat in cats : 
        ...
        if opt.doCategorization :
          ...
          if f.find("NMSSM") != -1:
            selection = "(genmbb >= %d and genmbb <= %d) and (MX_Y%d <= %.2f and MX_Y%d > %.2f) and (xmlMVAtransf <= %.2f and xmlMVAtransf > %.2f) and (ttHScore >= %.2f) and (Mjj >= %.2f and Mjj <= %.2f)" %(Mjj-2,Mjj+2,Mjj,float(MX_cut2),Mjj,float(MX_cut1),cat_def[cat]["MVA"][0],cat_def[cat]["MVA"][1],ttHScore,MjjLow,MjjHigh)
      ...
      f_out = ROOT.TFile.Open("%s/%s/%s_%s.root"%(opt.out_dir+sig+"_MXMgg/"+str(masses[i]),str(Mjj),target_names[num],year),"RECREATE")
