...
for mass in $(echo $Masses | sed "s/,/ /g")
    ...
    ./bin/fTest2D -i /eos/user/l/lata/Resonant_bbgg/flattrees_NMSSM_fromjobs/hadd_files/Run2_WS/$Signal/$mass/$massY/${name2D}.root  --saveMultiPdf  CMS-HGG_multipdf2D_${name2D}_${outtag}_HHbbgg_${YEAR}_${DATE}.root --isData 1 -f DoubleHTag_0,DoubleHTag_1,DoubleHTag_2 --isFlashgg 1  -c 3 -D ${outputdir} -d ${outputdir}/res.dat --jsonForEnvelope ../MetaData_HHbbgg/Env_json_02_09_2020.dat --massY $massY
    fi
    done
set +x
