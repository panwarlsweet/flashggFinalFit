...
//	RooAbsData *data = (RooDataSet*)inWS->data(Form("data_mass_%s",catname.c_str()));
	RooAbsData *data = (RooDataSet*)inWS->data(Form("Data_13TeV_%s",catname.c_str()));
	...
	//		if (!w_sig) w_sig = (RooWorkspace*)sigFile->Get("wsig_13TeV");
      ...
      if (w_sig) SignalType=1;
			...
			if (SignalType==1) {
				...
				RooArgList *singleHiggs_pdfs = new RooArgList();
				RooArgList *singleHiggs_pdfs_norm = new RooArgList();
				RooArgList *signal_pdfs = new RooArgList();
				RooArgList *signal_pdfs_norm = new RooArgList();
            ...
            double normalization_bbgg = 0.;
				...
				for (unsigned int pdf_num=0;pdf_num<signalName_.size();pdf_num++) {
					double intLumiYear = 1.;
				   if (signalName_[pdf_num].find("2016") != string::npos) intLumiYear=35.9;  
				   if (signalName_[pdf_num].find("2017") != string::npos) intLumiYear=41.5;  
				   if (signalName_[pdf_num].find("2018") != string::npos) intLumiYear=59.4; 
					double signal_norm_term =((RooRealVar*)w_sig->function(Form("hhbbggpdfsm_13TeV_%s_%s_norm",signalName_[pdf_num].c_str(),catname.c_str())))->getVal()*intLumiYear*1000.; 
					RooRealVar *rooSignorm =  new RooRealVar(Form("norm_sig_%s_%s",signalName_[pdf_num].c_str(),catname.c_str()),"",0.0,0.0,1.e6);
				...
				RooAddPdf* signalPdfSum = new RooAddPdf( "signalPdfSum", "signal", *signal_pdfs,*signal_pdfs_norm );

				for (unsigned int pdf_num=0;pdf_num<singleHiggsNames_.size();pdf_num++) {
					double intLumiYear = 1.;
				   if (singleHiggsNames_[pdf_num].find("2016") != string::npos) intLumiYear=35.9;  
				   if (singleHiggsNames_[pdf_num].find("2017") != string::npos) intLumiYear=41.5;  
				   if (singleHiggsNames_[pdf_num].find("2018") != string::npos) intLumiYear=59.4; 
					double singleHiggs_norm_term =((RooRealVar*)w_sig->function(Form("hhbbggpdfsm_13TeV_%s_%s_norm",singleHiggsNames_[pdf_num].c_str(),catname.c_str())))->getVal()*intLumiYear*1000.; 
					RooRealVar *rooSingleHnorm =  new RooRealVar(Form("norm_singleH_%s_%s",singleHiggsNames_[pdf_num].c_str(),catname.c_str()),"",0.0,0.0,1.e6);
				...
				RooAddPdf* singleHiggsPdfSum = new RooAddPdf( "singleHiggsPdfSum", "singleH", *singleHiggs_pdfs,*singleHiggs_pdfs_norm );
 

				singleHiggsPdfSum->plotOn(plot,Normalization(normalization_singleHiggs,RooAbsReal::NumEvent),LineColor(kBlue),LineWidth(3));
				singleHiggsPdfSum->plotOn(plot,Normalization(normalization_singleHiggs,RooAbsReal::NumEvent),LineColor(kBlue),LineWidth(3),FillColor(38),FillStyle(3001),DrawOption("F"));
				singleHiggsPdfSum->plotOn(plotLC,Normalization(normalization_singleHiggs,RooAbsReal::NumEvent),LineColor(kBlue),LineWidth(3));
				
				TObject *sigLeg = (TObject*)plot->getObject(plot->numItems()-1);
				leg->AddEntry(sigLeg,Form("Single Higgs m_{H} = %.1f GeV",MH->getVal()),"L");

				signalPdfSum->plotOn(plot,Normalization(normalization_bbgg*scaleSignal*scaleSignal_SM,RooAbsReal::NumEvent),LineColor(kViolet),LineWidth(3));//,AddTo(Form("first_%s",catname.c_str())));
				signalPdfSum->plotOn(plotLC,Normalization(normalization_bbgg*scaleSignal*scaleSignal_SM,RooAbsReal::NumEvent),LineColor(kViolet),LineWidth(3));

			//	std::cout << "[INFO] expected number of events in signal PDF " << signalPdfSum->expectedEvents(*MH) << std::endl;	
			//	std::cout << "[INFO] expected number of events in single Higgs PDF " << singleHig->expectedEvents(*MH) << std::endl;	

		    	TObject *sigLeg_bbgg = (TObject*)plot->getObject(plot->numItems()-1);
				leg->AddEntry(sigLeg_bbgg,Form("Signal SM x %.0f",scaleSignal),"L");

				outWS->import(*singleHiggsPdfSum);
				outWS->import(*signalPdfSum);
			}
		}
