...
int main(int argc, char *argv[]) {
 ...
 // RooRealVar *mass = win->var("Mjj");
  RooRealVar *mass = win->var("Mjj_90GeV");
  mass->SetTitle("#m_{jj}");
  mass->setUnit("GeV");
 ...
 // for (int i=0; i<chan->numTypes()-2; ++i) {
  ...
  for (int i=0; i<2; ++i) {
    ...
    desc.ReplaceAll("DoubleHTag_","CAT ");
 ...
 //   chan->setIndex(i);
  for (int i=0; i<2; ++i) {
    ...
    //TH1 *hstmp = new TH1D ( sbevents*(*hsbtmp) - bevents*(*hbtmp) ); //FIXME this behaviour seems to be broken in updated ROOT/combine versions..
...
//  double lowedge = 70.;
  ...
  TH1D *hsig = new TH1D("hsig","",3200,lowedge,highedge);
  TH1D *hwsig = new TH1D("hwsig","",3200,lowedge,highedge);
  
  TH1D *hbkg = new TH1D("hbkg","",3200,lowedge,highedge);
  TH1D *hwbkg = new TH1D("hwbkg","",3200,lowedge,highedge);

  TH1D *hbkgplot = new TH1D("hbkgplot","",nbins,lowedge,highedge);
  TH1D *hwbkgplot = new TH1D("hwbkgplot","",nbins,lowedge,highedge);
  
  TH1D *hsigbkg = new TH1D("hsigbkg","",3200,lowedge,highedge);
  TH1D *hwsigbkg = new TH1D("hwsigbkg","",3200,lowedge,highedge);   
...
//  for (int i=0; i<chan->numTypes()-2; ++i) {
 //   chan->setIndex(i);
  for (int i=0; i<2; ++i) {
    ...
    TH1D *hsigplot_cat = new TH1D(TString::Format("hsigplot_cat%s",catnames[i].Data()),"",3200,lowedge,highedge);
...
//  const int ncats = chan->numTypes();
 ...
 // for (int isig=0; isig<hsigplotsfine.size()-2; ++isig) {
  for (int isig=0; isig<ncats+2; ++isig) {
...
//  for (int i=0; i<(chan->numTypes()+2-2); ++i) {
  for (int i=0; i<(2); ++i) {
  ...
  //for (int icat=0; icat<(ncats+2-2); ++icat) {
 // for (int icat=0; icat<(ncats-2); ++icat) {
  for (int icat=0; icat<2; ++icat) {
    for (int ibin=0; ibin<nbins; ++ibin) {
    std::cout << "########################################################################" << std::endl;
...
//     pad2->SetBorderMode(0);
    ...
    mytext->DrawLatex(0.129+0.03,0.85,"HH#rightarrow#gamma#gammab#bar{b}");
