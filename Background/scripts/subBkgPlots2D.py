...
parser.add_option("-H","--high",type='int',default=70,help="Sqrt(S) COM energy for finding strings etc")
parser.add_option("-L","--low",type='int',default=190,help="Sqrt(S) COM energy for finding strings etc")
parser.add_option("--highblind",type='int',default=135,help="Blinding region - Down")
parser.add_option("--lowblind",type='int',default=115,help="Blinding region - Up")
parser.add_option("--variableName",type='string',default='CMS_hgg_mass',help="CMS_hgg_mass or Mjj")
...
for cat in range(ncats):
  ...
  print ""
  execLine = '$CMSSW_BASE/src/flashggFinalFit/Background/bin/makeBkgPlots2D -f %s -b %s -o %s/BkgPlots_cat%d.root --variableName %s -d %s -c %d -l \"%s\"  -L %d -H %d  --lowblind %d  --highblind %d  '%(options.flashggCats,options.bkgfilename,options.outDir,cat,options.variableName,options.outDir,cat,options.catLabels[cat],options.low,options.high,options.lowblind,options.highblind)
