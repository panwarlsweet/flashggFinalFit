...
for mass in $(echo $Masses | sed "s/,/ /g")
    do
    combineout="ResHHbbgg_datacard_BulkGravitonhh_X${mass}_${DATE}_${YEAR}"
    DATACARD="final_workspaces/final_datacards/cms_HHbbgg_datacard_BulkGravitonhh_X${mass}_${DATE}_${YEAR}.txt"

    DATACARDroot="final_workspaces/final_datacards/cms_HHbbgg_datacard_BulkGravitonhh_X${mass}_${DATE}_${YEAR}.root"
    ...
    combine higgsCombineResHHbbgg_datacard_BulkGravitonhh_X${mass}_${DATE}_${YEAR}.MultiDimFit.mH125.*.root  --snapshotName MultiDimFit -n $combineout  -M AsymptoticLimits -m 125. --saveWorkspace -t -1 --run=blind --X-rtd TMCSO_AdaptivePseudoAsimov=0 --X-rtd TMCSO_PseudoAsimov=0 --cminDefaultMinimizerStrategy 0 --cminFallbackAlgo Minuit2,Migrad,0:0.1 --X-rt MINIMIZER_freezeDisassociatedParams --X-rtd MINIMIZER_multiMin_hideConstants --X-rtd MINIMIZER_multiMin_maskConstraints --X-rtd MINIMIZER_multiMin_maskChannels=1 --freezeParameters allConstrainedNuisances -s -1 >&limit_BulkGravitonhh_X${mass}_${DATE}_${YEAR}.txt&
    done
set +x
