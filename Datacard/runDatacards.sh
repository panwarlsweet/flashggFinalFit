...
for mass in $(echo $Masses | sed "s/,/ /g")
    do
    SYSINPUTFILE="final_workspaces/${signal}/CMS-HGG_sigfit_MggMX_${YEAR}_${DATE}_${signal}X${mass}ToY${massY}H125.root"

##if running with no systematcis use any file with ws

    SIGNALFILE="final_workspaces/${signal}/CMS-HGG_sigfit_MggMX_${YEAR}_${DATE}_${signal}X${mass}ToY${massY}H125.root"

    DATAFILE="final_workspaces/${signal}/CMS-HGG_multipdf2D_MggMX_DoubleEG_ftest_${signal}_X${mass}_Y${massY}_HHbbgg_${YEAR}_${DATE}.root"
#NODESFILE="CMS-HGG_sigfit_MggMX_${YEAR}_${DATE}.root"
